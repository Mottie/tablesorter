  ...
  $.extend({
    ...
    tablesorter: new function() {
      ...
      function appendToTable(table, init) {
        ...
        if (!bodyCaches[0]) { return; } // empty table - fixes #206
        if (config.debug) {
        ...
        function appendNodesToTable(nodes)
        {
          ...
          for (var i = 0; i < totalRows; i++) {
            ...
            if (!config.appender || !config.removeRows) {
      ...
      function sortNodes(nodes, table)
      {
        ...
        var tc = table.config;
        var sortList = tc.sortList;
        var l = sortList.length;
        nodes.sort(function(node1, node2) {
          var normalized1 = node1.normalized;
          var normalized2 = node2.normalized;
          // cache is undefined here in IE, so don't use it!
          for (var i = 0; i < l; i++) { //TODO foreach.
            var columnIndex = sortList[i][0];
            var order = sortList[i][1];
            // fallback to natural sort since it is more robust
            var s = /n/i.test(getCachedSortType(tc.parsers, columnIndex)) ? "Numeric" : "Text";
            s += order === 0 ? "" : "Desc";
            var dir;
            if (/Numeric/.test(s) && tc.strings[columnIndex]) {
              // sort strings in numerical columns
              if (typeof (tc.string[tc.strings[columnIndex]]) === 'boolean') {
                dir = (order === 0 ? 1 : -1) * (tc.string[tc.strings[columnIndex]] ? -1 : 1);
              } else {
                dir = (tc.strings[columnIndex]) ? tc.string[tc.strings[columnIndex]] || 0 : 0;
              }
            }
            var sort = $.tablesorter["sort" + s](table, normalized1[columnIndex], normalized2[columnIndex], columnIndex, colMax[columnIndex], dir);
            if (sort) { return sort; }
          }
      ...
      function multisort(table) { /*jshint loopfunc:true */
        ...
        sortTime, cache;
        if (config.serverSideSorting || !config.cache[0]) { // empty table - fixes #206
          return;
        }
