...
ts.filter = {
	...
	regex: {
		...
		// data.cached = table cell text from cache, so it has been parsed
	...
	types: {
		...
		operators: function( c, data ) {
			if ( /^[<>]=?/.test(data.iFilter) ) {
				...
				cachedValue = ( parsed || parser.type === 'numeric' ) && !isNaN(query) && typeof data.cached !== 'undefined' ? data.cached :
		...
		range : function( c, data ) {
			if ( ts.filter.regex.toTest.test(data.iFilter) ) {
				...
				result = ( parsed || c.parsers[index].type === 'numeric' ) && !isNaN(range1) && !isNaN(range2) ? data.cached :
					isNaN(data.iExact) ? ts.formatFloat( data.iExact.replace(ts.filter.regex.nondigit, ''), table) :
					ts.formatFloat( data.iExact, table );
	...
	findRows: function(table, filters, combinedFilters) {
		...
		var len, $rows, rowIndex, tbodyIndex, $tbody, $cells, columnIndex,
			...
			query, notFiltered, searchFiltered, filterMatched, fxn, ffxn,
			regex = ts.filter.regex,
			c = table.config,
			wo = c.widgetOptions,
		...
		for (tbodyIndex = 0; tbodyIndex < $tbodies.length; tbodyIndex++ ) {
			...
			if (combinedFilters === '' || wo.filter_serversideFiltering) {
				...
				if ((wo.filter_$anyMatch && wo.filter_$anyMatch.length) || filters[c.columns]) {
					...
					if (regex.query.test(wo.filter_defaultType[c.columns] || '')) {
				...
				for (rowIndex = 0; rowIndex < len; rowIndex++) {
					...
					if (data.anyMatch) {
						...
						data.cached = c.cache[tbodyIndex].normalized[rowIndex].slice(0,-1).join(' ');
						filterMatched = null;
					...
					for (columnIndex = 0; columnIndex < c.columns; columnIndex++) {
						...
						if (data.filter) {
							data.cached = c.cache[tbodyIndex].normalized[rowIndex][columnIndex];
							// check if column data should be from the cell or from parsed data
							if (wo.filter_useParsedData || data.parsed[columnIndex]) {
								data.exact = data.cached;
							} else {
							...
							if (fxn) {
								if (fxn === true) {
									...
									result = fxn(data.exact, data.cached, data.filter, columnIndex, $rows.eq(rowIndex));
								} else if (typeof fxn[ffxn || data.filter] === 'function') {
									// selector option function
									result = fxn[ffxn || data.filter](data.exact, data.cached, data.filter, columnIndex, $rows.eq(rowIndex));
								}
							} else {
								filterMatched = null;
